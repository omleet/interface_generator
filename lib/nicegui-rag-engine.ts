...
async function indexStreamlit(onProgress?: StreamlitProgressCallback): Promise<void> {
  ...
  try {
    // Fetch via the Next.js API proxy (/api/nicegui-docs) to avoid CORS.
    // Note: the route name is kept for backwards compat; it now fetches Streamlit docs.
    // The proxy fetches from docs.streamlit.io server-side and caches for 1 hour.
    const res = await fetch('/api/nicegui-docs')
    if (!res.ok) {
